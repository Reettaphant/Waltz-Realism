...
function getWorldEvents(numberOfTurns, initialStates, continuationOfOld, oldWorld, lastEvent, previously, bipolarCounter){
    ...
    World.prototype = {
        ...
        multipolarAlliances: function(){
            ...
            while (states.length != 0){
                ...
                if (oldCoalitions != 0 && oldCoalitionNumber != undefined && oldCoalitions[oldCoalitionNumber] != undefined){
                    this.addRestInCoalition(oldCoalitions, oldCoalitionNumber, addingCoalition, averagePower, largestOverAverage, states); 
                }
            }
        ...
        addRestInCoalition: function (oldCoalitions, oldCoalitionNumber, addingCoalition, averagePower, largestOverAverage, states){
    ...
    function setFirstHegemonFlag(h){
    ...
    function setHegemonFlags(world){
    ...
    function setScalingFlags(world){
    ...
    function setWorldPolarity(world){
    ...
    function setBuckpassFlags(world){
    ...
    function setAllianceNumbers(world){
    ...
    function generateWar(world){
        if  (turn.polarity == 'multipolar'){
            return new MultipolarWar(world); 
        }
        else if (turn.polarity == 'bipolar'){
            return new BipolarWar(world); 
        }	
        else{ 
            return new  UnipolarWar(world);
        }
     }

    function checkBipolarUnipolar(){

        if (turn.polarity == 'bipolar'){
            setBipolarFlags(h);
        }
        if (turn.polarity == 'unipolar'){
            setUnipolarHegemon(h);
        }
     }

    function setWorldWarFlags(world){
    ...
    function generateIntervention(world) {
    ...
    function setEndPolarityFlags(world){
    ...
    function setTerritoryIntelligence(world){
    ...
    var debug = false;
    var initialisation = initialiseEvents(); 
    var world = initialisation[0];
    var events = initialisation[1];
    for (var h=0; h<numberOfTurns; h++){
        ...
        setHegemonFlags(world);
        world.scale(); 
        var afterScale = packStates(world.states); 
        turn.statesAfterScaling = afterScale.slice();
        setScalingFlags(world);	
        setWorldPolarity(world);
        world.update(); 
        setUpdateFlags(world);
        world.deleteStates(); 
        ...
        setFirstHegemonFlag(h);
        checkBipolarUnipolar();
        if (h != 0){
        ...
        setBuckpassFlags(world);	
        setAllianceNumbers(world);
        var testWar = generateWar(world);
        testWar.calculateWar();
        turn.war = testWar.limitedWar;
        turn.escalation = testWar.escalation;
        setWorldWarFlags(world);
        pushWinners(testWar);
        if (turn.polarity == 'multipolar'){
            testWar.updateWorld(); 
        }
        ...
        if (world.decliningHegemon == true){
            generateIntervention(world);
        }
        setEndPolarityFlags(world);
        events.push(turn); 
    ...
    var terrIntel = setTerritoryIntelligence(world);
