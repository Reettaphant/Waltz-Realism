...
$(document).ready(function(){
	...
	/*functions that run the simulation*/
		...
		/*fetches quizzes from the server and displays them*/
			...
			/*restores the state the state of the simulation in case old turns need to be seen again*/
			...
			/*functions for visualising a unipolar world*/
			...
			/*animates the change in polarity of a bipolar world*/
			...
			/*animates the disintegration of a bipolar world*/
			...
			/*animates the outcomes of proxy wars in a bipolar world*/
			...
			/*animates proxy wars in a bipolar world*/
			...
			/*displays the alliances in a bipolar world*/
			...
			/*deals with seeing older turns again*/
			...
			/*click functions for going back a turn, seeing a turn again, seeing the next turn or exiting*/	
			...
			/*transitions to next turn in the simulation*/
			...
			function toNewTurn(){
			/*sets control variables to false before moving to the next turn*/
			...
			function limitedFix(){
			/*animates a systemic change that takes place after a limited war*/
				...
				function addStories(state, stateNumber, power){
					...
					if (power != events[j].statesAfterUpdate[stateNumber-1][0]){
						...
						if (disappear == true){
							...
							removeWithRegext(state, /^alliance|^sphere/); 
							$(state).addClass('alliance0').addClass('hidden').removeClass('visible');	
						}
						else{	
			...
			/*displays stories for fighting a war*/
			...
			/*animates the escalation of a war*/
				...
				if (events[j].flags.worldWar == false){
					...
					if (escalationVisit == false){
						...
						for (var k=0; k<attackers.length; k++){ 
							$(attackers[k]).addClass('attacker');
							$(attackers[k]).find('.attackImage').removeClass('hidden').addClass('visible'); 
						}
						...
						for (var k=0; k<defenders.length; k++){	   
							$(defenders[k]).addClass('defender');
							$(defenders[k]).find('.defenderImage').removeClass('hidden').addClass('visible'); 
						}
			...
			function assessWars(){
			/*animates states starting wars*/

				function peacefulWorld(){
			...
			function thinkWar(){
			/*animates states thinking of going to war*/
			...
			/*animates potential buckpassing*/
			...
			/*updates alliances in the world before a new turn starts*/
			...
			/*updates the powers of states as a new turn starts for all polarities*/
			...
			function scaling(){
			/*scales down the powers of the states as a new turn starts*/
			...
			/*sets up the descirptions of a state when a mouse hovers on top */
			...
			/*sets things ready to start the simulation*/
			...
			/*if powers were not specified sets the world in a uni-, bi-, or mulitpolar world automatically, choosing state powers randomnly*/
